...
namespace backtradercpp {
    namespace feeds {
        ...
        /**
         * At least override `read()`. `reset()` may also needed.
         * @tparam T
         */
        ...
        protected:
            ...
            // if is_degenerated=true, then only close price is valid.
            // this can be useful for acceleration.
        ...
        public:
            ...
            explicit BasePriceDataImpl(int n_assets)
                : GenericDataImpl(), assets_(n_assets) {
                codes_.reserve(n_assets);
                for (int i = 0; i < n_assets; ++i)
                {
                    codes_.emplace_back("A" + std::to_string(i));
                }
                next_.resize(n_assets);
            }
