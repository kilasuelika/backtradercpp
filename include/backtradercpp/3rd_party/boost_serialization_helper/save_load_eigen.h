...
inline void serialize(Archive &ar, Eigen::Array<S, Rows_, Cols_, Ops_, MaxRows_, MaxCols_> &arr,
                      const unsigned int version) {
    int rows = arr.rows();
    int cols = arr.cols();
    ar &make_nvp("rows", rows);
    ar &make_nvp("cols", cols);
    arr.resize(rows, cols); // no-op if size does not change!

    // always save/load row-major
    for (int r = 0; r < rows; ++r)
        for (int c = 0; c < cols; ++c)
            ar &make_nvp("val", arr(r, c));
}
